...
def calculate_td_sequential(data):
    ...
    # Add logic for perfected setups
    perfected_buy = []
    perfected_sell = []
    for y in range(len(c)):
        if buyVal[y] == 9:
            if y >= 8 and (l[y] <= l[y-6] and l[y] <= l[y-7] or l[y-1] <= l[y-6] and l[y-1] <= l[y-7]):
                perfected_buy.append(True)
            else:
                perfected_buy.append(False)
        else:
            perfected_buy.append(False)

        if sellVal[y] == 9:
            if y >= 8 and (h[y] >= h[y-6] and h[y] >= h[y-7] or h[y-1] >= h[y-6] and h[y-1] >= h[y-7]):
                perfected_sell.append(True)
            else:
                perfected_sell.append(False)
        else:
            perfected_sell.append(False)
    ...
    return t, o, h, l, c, v, vol, longVal, shortVal, buyVal, sellVal, buy_deferred, sell_deferred, buy_recycled, sell_recycled, agbuyVal, agsellVal, \
        nineEMA, tenEMA, twelveEMA, twentysixEMA, thirtyEMA, fiftyEMA, sixtyEMA, onetwentyEMA, perfected_buy, perfected_sell
...
def update_graph(n_clicks, ticker, period, volume_toggle):
    ...
    t, o, h, l, c, v, vol, longVal, shortVal, buyVal, sellVal, buy_deferred, sell_deferred, buy_recycled, sell_recycled, agbuyVal, agsellVal, \
        nineEMA, tenEMA, twelveEMA, twentysixEMA, thirtyEMA, fiftyEMA, sixtyEMA, onetwentyEMA, perfected_buy, perfected_sell = calculate_td_sequential(data)
    ...
    for z in range(len(c)):
        ...
        # Add perfected setup annotations
        if perfected_sell[z]:
            fig.add_annotation(
                x=t[z], y=h[z]*1.02,
                text='P9',
                arrowhead=2, arrowsize=1, arrowwidth=2,
                arrowcolor='#FF0000', ax=0, ay=-50,
